...
Access is gated on Cognito groups: `SentinelNetAdmins`, `SentinelNetAnalysts`, `SentinelNetViewers`. Viewers get a 403 from TheHive/Grafana/Wazuh and should consume telemetry through the dashboard pages instead.

---

## Access Management Terminal

The website includes an admin-only **Access Management Terminal** at `/admin/access`. It's a terminal-style UI but **not** a real shell — it translates a small approved command set into Cognito admin API calls.

Supported commands: `help`, `list-users`, `get-user <email>`, `grant-access <email> <Admins|Analysts|Viewers>`, `revoke-access <email> <Admins|Analysts|Viewers>`, `list-groups`, `whoami`, `clear`.

Only users in `SentinelNetAdmins` can access the page or call the admin access API. The Lambda backing it (`backend/lambda/admin_access_api/`) reads `USER_POOL_ID`, `ADMIN_GROUP_NAME`, `ANALYST_GROUP_NAME`, `VIEWER_GROUP_NAME` from its env.

After changing the frontend, redeploy so CloudFront serves the new page and runtime config.
