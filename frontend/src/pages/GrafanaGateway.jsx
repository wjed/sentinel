...
export default function GrafanaGateway() {
  const auth = useAuth()
  const [showNoAccess, setShowNoAccess] = useState(false)

  const handleLaunch = () => {
    if (!auth.isAuthenticated) {
      // Redirect to Cognito first, then come back here
      auth.signinRedirect({ state: { returnTo: '/grafana' } })
      return
    }

    // Check if user has groups (basic access check)
    ...
    // Open Grafana via ALB (ALB will handle Cognito auth transparently)
    window.open(ALB_GRAFANA, '_blank')
  }
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        ...
        >
          ...
          <div style={{
            ...
            background: 'linear-gradient(135deg, #FF6B00 0%, #F9B100 100%)',
            borderRadius: '16px',
        ...
        >
          ...
          >
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              You don't have permission to access Grafana.
